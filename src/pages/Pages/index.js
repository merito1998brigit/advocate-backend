...
class BannersList extends React.Component {
  ...
  handleDelete = async (id) => {
    console.log('id', id)
    const isDeleted = await callApi(`/api/backend/v1/pages/${id}`, { method: 'DELETE' })
  ...
  handleMenuClick = async e => {
    ...
    const isUpdated =await callApi(`/api/backend/v1/pages/${clickedId}?status=${e.key}`,{method:'PATCH'})
  ...
  onCellChange = (id, key) => {
    return async value => {
      ...
      const edited = await callApi(`/api/backend/v1/pages/${id}?priority=${value}`,{method:'PATCH'})
      if (edited) {
        notification.success({ message: STRINGS.success,description:STRINGS.editSuccess})
        return true
      }
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        key: 'image',
        render: (text, record) => (
          <Link to={`/pages/edit/${record.id}`} className="thumbnail-area">
